...
def draw():
    ...
    if game_over:
        screen.fill('green')
        screen.draw.text('Eindscore: ' + str(score), color='black', center=(700, 400), fontsize=60)
        screen.draw.text('Druk op spatie om te herstarten', color='black', center=(700, 430), fontsize=30)
        if keyboard.space:
            game_over = False
            score = 0
            tijd_over = tijd
